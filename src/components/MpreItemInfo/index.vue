    ...
    <div ref="listCt" class="l-list-ct" style="position: relative!important;">
    	<div v-if="preCode||typeof preCode != 'undefined'" class="l-list-title clearfix"> 
    		...
    		<div class="title-right fr" v-if="status!=1">
    			<span class="now-num vux-1px-r">现有库存{{nowNum}}</span><span class="safe-num">安全库存 <span>{{safeNum}}</span></span>
    		</div>
    	</div>
    		<div class="l-list-content vux-1px-t">
    			...
    			<div class="l-list-name l-list-nametext">
    				{{name}}
    				<slot name="textNumDom">
		    			<div v-if="totalNum" class="l-list-text">
		    				x{{totalNum}}
		    			</div>
	    			</slot>
    			</div>
