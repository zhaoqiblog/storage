    ...
    <div class="contentWrap">
    	...
    	<div class="storeSelect">
    		<tab :line-width="2" custom-bar-width="60px">
    			<!--:selected="index==0"-->
			     <tab-item :selected="initItem == item.index" v-for="(item, index) in tablist" :key="index"  @on-item-click="onItemClick">{{item.name}}&nbsp;&nbsp;{{item.value}}</tab-item>
      	</tab>
	      	<div class="wipeList" style="margin-top: 0;">
...
//	this.swipeHeight=this.dataLists.datas.length*110+55+'px'
  ...
  methods: {
  	onItemClick(index){
			if(index==1){
				this.listData=this.initData.intarger;
			}
//			else{
////				this.
//			}
...
//					[{name:"散货商品",value:0,index:1},{name:"整件商品",value:0,index:2}],
					...
					if(e.floats.length<1){
						this.tablist=[{name:"整件商品",value:e.intarger.length,index:0}];
						this.listData=this.initData.intarger;
					}else if(e.intarger.length<1){
						this.overall=0;
						this.listData=this.initData.floats;
						this.tablist=[{name:"散货商品",value:e.floats.length,index:0}]
					}else{
						this.listData=this.initData.floats;
						this.tablist=[
							{name:"散货商品",value:e.floats.length,index:0},
							{name:"整件商品",value:e.intarger.length,index:1}
						]
//						this.tablist[0].value=e.floats.length;
//						this.tablist[1].value=e.intarger.length;
					}
