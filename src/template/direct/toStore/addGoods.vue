<template>
  <div class="self-process">
    <x-header backScan>添加商品</x-header>
    ...
    <!--</scroller>-->
    	 ...
    	 <div class="cm-footer">
	      ...
	      <div class="btn-submit">  <!--:disabled=""-->
	        <button type="button" @click="gotoConfirm" :disabled="disableds">确认入库清单</button>
	      </div>
    	</div>
  </div>
</template>
...
export default {
  ...
  computed: {
  	...mapState({
  		...
  		directToStoreAdd:state=>state.direct.directToStoreAdd,
  ...
  methods: {
    ...
    addGoods(){
		 ...
		 * */
    gotoConfirm(){
			this.$store.commit("setdirectToStoreList",this.initData.filter(i=>{return i.count>0}))
			this.$store.commit("setdirectToStoreAdd",this.initData)
			console.log(this.directToStoreList)
			this.$router.push({path:'directToStoreConfirm',query:{scanResult:this.$route.query.scanResult}})
    },
    /**
     * 数据变化
     * */
