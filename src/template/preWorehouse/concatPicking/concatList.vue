<template>
	<div class="pre-picking" style="overflow-x: auto;">
		...
		<x-header class="vux-1px-b" :left-options="{preventGoBack:true}" @on-click-back="back">
			合单拣货
			<a slot="right">
				<router-link to='searchOrder'>搜索</router-link>
			</a>
		</x-header>
		<!--</div>-->
		...
		<div class="scroll-content pre-pick-list pre-concat-list" ref="scrollWrap">
			...
			<div v-if="index==0">
				<div class="container-list">
					<pre-list-concats v-for="(e,index) in data.noPick" :key='index' :id="e.id" :classInfo="e.classInfo" :isOver="e.isOver"
					 :outerOrderType="e.outerOrderType" :creatTime="e.creatTime" :OverText="e.OverText" :skuNum="e.skuNum"
					 :orderSequenceNo="e.orderSequenceNo" :deliverType="e.deliverType" :slotType="e.slotType" :receiveOrder="receiveOrder"></pre-list-concats>
				</div>
			...
			<div v-if="index==1">
				<div class="container-list">
					<pre-list-concats v-for="(e,index) in data.overTime" :key='index' :id="e.id" :classInfo="e.classInfo" :isOver="e.isOver"
					 :outerOrderType="e.outerOrderType" :creatTime="e.creatTime" :OverText="e.OverText" :skuNum="e.skuNum"
					 :orderSequenceNo="e.orderSequenceNo" :deliverType="e.deliverType" :slotType="e.slotType" :receiveOrder="receiveOrder"></pre-list-concats>
				</div>
						...
						<div class="pre-list-item-content">
							...
							<label :for="'radios'+index" class="radisLabel" style="width: 100%;height: 100%;position: absolute;top: 0;left: 0;">
								<div class="button-to-pick">
									...
									<input type="checkbox" name="radios" :id="'radios'+index" v-model="selectLists" :value="e.id" @change="changeSeleList('picking')"
						...
						<div class="pre-list-item-content ">
							<div>
								<dl>
									...
									<dd class="unPageText">
										<span class="category-num" v-for="(key,ind) in e.mergeOrderSequenceNo" :key="ind+1">{{key}} <span v-if="ind<e.mergeOrderSequenceNo.length-1">,</span></span>
									</dd>
								</dl>
							</div>
							<div class="button-to-pick">
		...
		<div class="pre-footer" v-if="index==2">
			<div class="btn-add btn-tips">
				...
				<label for="selectDefault">&nbsp;&nbsp;默认{{concatNumber}}单，可多选</label>
			</div>
		...
		methods: {
			...
			console.log(this.timer)
				let obj = {
					...
					status: 0,
					page: this.pageOver.pageNo,
					size: this.pageOver.pageSize,
				}
				if (isUnpackage) { //未装袋时：
			 ...
			 */
			changeSeleList(val) {
				if (this.selectLists.length != Number(this.concatNumber)) {
			...
			/*开始拣货*/
			startPick() {
				$request.post("/api/online-order/v1/protected/whether/outlimit", this.selectLists).then(res => {
					if (res.success == true) {
						clearInterval(this.timer);
						this.$router.push({
							path: 'concatPicking',
							query: {
								'id': this.selectLists.join("|")
							}
						});
					} else {
						this.$vux.toast.show({
					...
					if (res.success) {
						...
						if (status == 0) { //未拣货
							if (pageNow == 1) { //第一页
								...
								// console.log(this.refreshTime,this.timer)
								if(!this.timer&&this.refreshTime){
									...
									},this.refreshTime?parseInt(res.data.refreshTime)*1000:15000)
								}
							}
							res.data.content.map((item, ins) => {
								let supplyNum = 0,
